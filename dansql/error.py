from dansql.position import Position, Token
from dansql.token import Position, Token
from dansql.ansi_colors import ANSIColors
  ...
  def generate_error_text(self) -> str:
    res: str = (
        ANSIColors.bright_red(
            f"\n{self.type}" + "\n @ File" + f" {self.src_path}, {str(self.start_pos)}"
        )
        + "\n"
    )
...
class ParserError(Error):
  ...
  def __init__(self, src_path: str, msg: str, token: Token):
    super().__init__(
        src_path,
        msg,
...
class LexerError(Error):
  ...
  def __init__(
      self, src_path: str, msg: str, src: str, start_pos: Position, end_pos: Position
  ):
    super().__init__(src_path, msg, src, start_pos, end_pos, "Lexer Error")
